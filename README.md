...
与另外两部百科一样，引用 Bot 发出的盟友消息后直接发送 `卡比真格`，插件会优先使用该盟友的英文 `page_title` 查询；无法唯一定位页面时会列出中英日候选，避免误把不同作品的 Boss 版本混在一起。
...
| `kirby_catalog_lookup_shinkaku` | 使用中文、英文或日文名称查询真格攻略 Wiki 页面和指定日文攻略栏目 |
