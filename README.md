...
与另外两部百科一样，引用 Bot 发出的盟友消息后直接发送 `卡比真格`，插件会优先使用该盟友的英文 `page_title` 查询；无法唯一定位页面时会列出中英日候选，避免误把不同作品的 Boss 版本混在一起。候选列表默认按合并转发发送，每个候选独占一个节点；可通过“真格多候选页面回复形式”切回普通消息。
