...
| 模块 | 能力 |
| --- | --- |
| 盟友抽取 | 每日限次抽取、简体中文简介、可配置内容编排、冷却时间、连续未出新保底、纯文本 `今日盟友` 触发 |
...
只有 `今日盟友`、`随机盟友` 和 `查盟友` 会附带简介。猜盟友、图鉴长图、名单和排行榜不会附带简介，避免无关消息变长。
...
| 配置组 | 常用选项 |
| --- | --- |
| 抽取与图鉴 | 每日抽取次数、今日/随机/查询文案、简介和百科提示开关、详情编排、冷却时间、猜盟友超时、图鉴列数 |
...
- `今日盟友`、`随机盟友` 和 `查盟友` 默认显示内置或人工简介；简介与百科提示均可单独关闭。
