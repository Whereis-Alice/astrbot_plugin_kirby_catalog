...
从 `v4.3.0` 起，视口宽度在 `1361px` 以上时七个页面全部一屏满铺、页面本身不滚动，长列表在各自的分区内滚动，打开管理台即可看到整页的全部功能；`1366×900` 这类小尺寸笔记本屏也仍是一屏。`1360px` 及以下自动回落为普通文档流，打印时同样。数据表行高统一为 `66px`（素材库带缩略图，为 `67px`）；窗口窄于表格下限时，表格区域可以左右滚动查看被挤出的列。
...
`v4.3.0` 把四个样式表和 `index.html` 全部重写，模块划分与 DOM 结构契约保持不变，因此视图脚本几乎没有改动。样式分层的职责是：`tokens.css` 只放变量（三款皮肤各自的配色、渐变、光斑、噪点与玻璃背板），`base.css` 放骨架与顶栏，`components.css` 放可复用组件（指标卡、表格、徽章、抽屉、弹层、表单），`views.css` 放七个视图各自的版式与一屏满铺规则。
