...
插件以“年代编号 + 规范名称 + 本地素材”为核心管理盟友，适合数百张以上、仍会持续扩充的素材库。它支持每日抽取、个人与群图鉴、进度排行、猜盟友、引用改名或换图，以及 WiKirby 和 Kirby Wiki | Fandom 双百科查询。
...
| 收藏图鉴 | 按首次登场作品排序的固定编号、个人与群图鉴、名称显示、有效进度和群内排行榜 |
...
`migrate_catalog.py` 用于把已经运行过的旧图鉴整体迁移到一套新的 WiKirby 规范素材。它不会从网络下载图片；新素材目录必须提前准备好以下内容：
...
本项目 v3 基准清单包含 1092 个来源页面。迁移规则会排除物件、机制、地点、组织、种族或类型汇总等 14 个非角色页面，最终建立 1078 个角色条目；每个排除项及原因都会写入迁移报告，避免素材悄悄消失而无法复核。

迁移后的编号首先按角色最早登场作品的年份从早到晚排列。同一年内按发行映射文件声明的作品顺序排列；同一作品内优先列出主角卡比，再按角色类型和 WiKirby 页面标题稳定排序。没有可靠首作年份的条目排在最后，并在 `新图鉴编号.csv` 中标记为“待确认”。
...
以 AstrBot 安装在 `/root/AstrBot`、新素材位于 `/Kirby` 为例，先停用插件，再进入插件目录执行预演：
...
| 报告 | 用途 |
| --- | --- |
| `migration_summary.json` | 新旧条目、解锁恢复率、展开和漏迁总数 |
| `旧素材匹配报告.csv` | 每个旧条目的匹配方法、目标和候选项 |
| `漏迁用户记录.csv` | 无法恢复的群、用户、素材和解锁日期 |
| `用户迁移影响.csv` | 每位用户迁移前后的解锁数及去重影响 |
| `新图鉴编号.csv` | 新编号、首作年份、作品、名称和文件名 |
| `已排除非角色页.csv` | 未进入新图鉴的来源页面、文件名和排除原因 |
| `迁移复核.html` | 低置信度、一对多和未匹配条目的图片对照页 |

匹配顺序包含人工覆盖、图片 SHA-256、规范名称、感知哈希和角色归并。组合图片可以人工展开到多个规范角色；多个旧条目落到同一新角色时，只保留该用户最早的解锁日期。无法可靠对应的旧条目只写入漏迁报告，不会随意分配给相似图片。

> [!NOTE]
> “成功映射的历史记录数”和“迁移后的有效解锁数”不是同一个指标。旧素材中的卡比能力形态、别名、重复图或同一角色的多个造型会归并到一个规范角色，因此历史记录可以找到去向，但个人进度可能因规范去重而减少。请在正式执行前检查 `用户迁移影响.csv` 中的 `old_unlock_rows`、`mapped_unlock_rows`、`new_unique_unlocks` 和 `unresolved_unlock_rows`。
...
> [!IMPORTANT]
> 活动目录中的 `img/allies` 只包含新素材。完整旧目录会自动重命名为同级的 `astrbot_plugin_kirby_catalog.before-v3-时间戳`，旧素材仅留在该备份中供回滚。不要在确认迁移结果前删除备份。
...
### 为什么 v3 迁移后的解锁数量比旧图鉴少？

v3 按规范角色计数，不再把同一角色的能力形态、别名、重复图和多个旧造型分别算作不同角色。迁移工具会保留每个可识别记录的去向和最早解锁日期，再按规范角色去重；具体归并和漏迁数量可查看 `用户迁移影响.csv` 与 `漏迁用户记录.csv`。
