...
从 `v3.4.0` 起，插件提供 AstrBot Dashboard 原生管理 Page。更新并重载插件后，在 Dashboard 的插件详情中打开“星之卡比图鉴管理台”即可使用，不需要另开端口或部署独立前端。`v4.0.0` 对这套前端做了完全重置：改为模块化 ES module 与分层样式、四款新皮肤和内联 SVG 图标；管理功能、管理 API 和数据格式保持不变，升级后无需迁移数据或调整配置。`v4.3.0` 又把视觉层整体重置了一遍：悬浮玻璃顶栏、带滑动指示条的胶囊导航、渐变标题、24px 面板圆角，三款皮肤全部重新调色，概览页重排为一屏四段的仪表盘。两次重置都只改前端表现，管理功能、管理 API 与数据格式始终不变。`v4.4.0` 又加了第八个页面「数据备份」，负责各类数据与素材图片的导出、预检和导入。
...
`v4.4.0` 新增「数据备份」页，可以把管理台里的数据导出成文件，也可以从备份文件恢复到当前这台机器。七类数据彼此独立，既能只导一类，也能整机迁移。
...
从 `v4.3.0` 起，视口宽度在 `1361px` 以上时八个页面全部一屏满铺、页面本身不滚动，长列表在各自的分区内滚动，打开管理台即可看到整页的全部功能；`1366×900` 这类小尺寸笔记本屏也仍是一屏。`1360px` 及以下自动回落为普通文档流，打印时同样。数据表行高统一为 `66px`（素材库带缩略图，为 `67px`）；窗口窄于表格下限时，表格区域可以左右滚动查看被挤出的列。

### 前端结构

管理台前端在 `v4.0.0` 完成重写，由 26 个原生 ES module 组成，没有构建步骤和打包产物：
...
图标改为 102 个内联 SVG，不再随插件打包第三方图标脚本，首屏没有任何外部图标请求，也不依赖 CDN。插件 logo 的矢量版也内联在顶栏里，同样不产生额外请求。

`v4.3.0` 把四个样式表和 `index.html` 全部重写，模块划分与 DOM 结构契约保持不变，因此视图脚本几乎没有改动。样式分层的职责是：`tokens.css` 只放变量（三款皮肤各自的配色、渐变、光斑、噪点与玻璃背板），`base.css` 放骨架与顶栏，`components.css` 放可复用组件（指标卡、表格、徽章、抽屉、弹层、表单），`views.css` 放八个视图各自的版式与一屏满铺规则。
