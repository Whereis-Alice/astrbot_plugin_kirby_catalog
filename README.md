...
群内发送 `卡比真格速查` 或 `卡比真格速查图` 会生成同一份索引的 PNG 速查图。默认把 301 个页面排成一张 `2160px` 宽的四列大字号总览图：作品与栏目标题横跨整行，每项显示 `#目录编号`、简体中文和日文页面名，组内不重复显示作品后缀。默认完整图约为 `2160×7582`、`16.38MP`、`1.40MiB`；生成结果缓存于 `data/plugin_data/astrbot_plugin_kirby_catalog/shinkaku_reference`。需要进一步放大时，可以关闭单张模式恢复旧版分页图。
...
| 配置项 | 默认值 | 说明 |
| --- | ---: | --- |
| `shinkaku_reference_single_image` | 开启 | 把全部 301 个页面合并为一张紧凑总览图；关闭后恢复旧版分页图 |
| `shinkaku_reference_compact_columns` | `4` | 单张总览图列数；范围 `4-7`。四列使用最大字号；列数越多图片越短，单列文字越窄 |
...
速查图不会改变目录编号。无论使用单张还是分页模式，`#1-#301` 始终按“作品 -> 原站栏目 -> 原站菜单顺序”排列；`卡比真格文档 88` 使用的就是这套编号。升级前若配置已保存为五列，插件也会使用比旧版更大的字号；改为四列可获得默认的最大字号。旧版分页模式若最后一页只剩一列条目，会自动并入前一页，避免额外发送一张近乎空白的图片。
