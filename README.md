...
真格页面中的能力、评价、实机记录等 HTML 表格不会再压成一段普通文字。选择卡片回复时，插件会保留表头、完整行和页面原有的 Seesaa 图标；主模块、折叠子模块和表格按网页原始顺序排列，表格始终留在所属模块内。开启“使用 LLM 将真格攻略日文翻译成简体中文”后，会翻译栏目名、表头和自然语言单元格，同时保留 `SS`、`A`、数字、实机时间和 `--` 等原始数据。表格太长时按完整行分页，后续卡片会重复表头。
...
- **文档版**：生成一个自包含的响应式 HTML 文件。它会读取当前页面全部可解析栏目，不受对应百科“显示详细资料”开关影响，并保留模块顺序、子标题、文字强调、表格、语录、招式、首图、真格攻略图标、来源链接和相关媒体。手机端技能资料自动改为单列，宽表格只在表格区域横向滚动。
...
- 超长正文会按 `wiki_translation_chunk_chars` 分块；Fandom 语录、招式和真格攻略表格还会继续按栏目、语录、招式组或完整行拆分为多批 JSON，避免模型截断整张长表；
...
名称库修改后会自动生成新的 revision，旧翻译缓存不会误复用。插件会自动修复模型对占位符造成的 Markdown 加粗、反斜杠转义等可逆变形；同一个已知占位符被多输出时也能安全恢复。只有模型漏掉专名或凭空生成未知占位符时，该批内容才回退为已经规范化名称的原文，不会把错误机翻名称写进结果。没有开启 LLM 翻译时，默认仍会执行一次快速名称规范化，不产生模型调用。
...
QQ 纯文本无法保留复杂表格布局，合并转发仍受节点上传稳定性影响，超长卡片则可能生成多张高像素图片。`卡比百科文档`、`卡比F文档` 和 `卡比真格文档` 会把完整可解析内容放入一个响应式 HTML 文件：正文可复制，链接可点击，宽表格可横向滚动，也不会因图片分页而制造多条群消息。需要快速浏览时仍可使用卡片；需要完整保存、检索或核对数据时优先使用文档。
