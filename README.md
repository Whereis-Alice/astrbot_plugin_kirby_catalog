...
真格页面中的能力、评价、实机记录等 HTML 表格不会再压成一段普通文字。选择卡片回复时，插件会保留表头、完整行和页面原有的 Seesaa 图标；开启“使用 LLM 将真格攻略日文翻译成简体中文”后，会翻译栏目名、表头和自然语言单元格，同时保留 `SS`、`A`、数字、实机时间和 `--` 等原始数据。表格太长时按完整行分页，后续卡片会重复表头。
...
- **卡片版**：使用当前百科设置的模板生成图片。短页面保持单张，真正过长时按栏目、语录、招式组或完整表格行分页，不截断正文。
- **文档版**：生成一个自包含的响应式 HTML 文件。它会读取当前页面全部可解析栏目，不受对应百科“显示详细资料”开关影响，并保留表格、语录、招式、首图、真格攻略图标、来源链接和相关媒体。手机可横向滚动宽表格，电脑可直接铺开阅读。
...
普通页面仍只生成一张卡片。只有内容预计过长，或渲染后的宽高、总像素、文件大小超过安全阈值时，插件才会自动分页。分页按简介续篇、资料栏目、语录条目和表格行拆分；每页重复页码与来源，招式表和真格攻略表的续页会保留表头，正文和表格行不会被截断。默认安全阈值为宽 `2160px`、高 `8000px`、`18MP` 和 `8MiB`，这些是针对 NapCat/QQNT 的保守兜底，不是 QQ 官方公布的硬限制。
