...
| 模块 | 能力 |
| --- | --- |
| 盟友抽取 | 每日限次抽取、可配置抽取文案、冷却时间、连续未出新保底、纯文本 `今日盟友` 触发 |
...
| 配置组 | 常用选项 |
| --- | --- |
| 抽取与图鉴 | 每日抽取次数、抽取文案模板、冷却时间、猜盟友超时、图鉴列数 |
...
- 默认抽取文案显示盟友名、图鉴编号、首次登场作品和剩余次数；可在 WebUI 的多行 `draw_message_template` 中使用 `{nickname}`、`{name}`、`{id}`、`{source}`、`{source_text}`、`{flags}`、`{remaining}` 自定义。无效模板会自动回退到默认文案。
- `随机盟友` 和 `猜盟友` 不写入用户图鉴。
