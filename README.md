...
群内发送 `卡比真格速查` 或 `卡比真格速查图` 会生成同一份索引的 PNG 速查图。图片按作品和栏目分组，每项同时显示 `#目录编号`、简体中文和日文页面名；生成结果缓存于 `data/plugin_data/astrbot_plugin_kirby_catalog/shinkaku_reference`。
...
| 配置项 | 默认值 | 说明 |
| --- | ---: | --- |
| `shinkaku_reference_entries_per_page` | `50` | 速查图每页显示的页面条目数；范围 `20-200`。数值越大，单张图片越长 |
| `shinkaku_reference_columns` | `2` | 速查图列数；范围 `1-3`。页面名较长或需要更大字号时可改为 `1` |

速查图不会改变目录编号。无论每页显示多少条，`#1-#301` 始终按“作品 -> 原站栏目 -> 原站菜单顺序”排列；`卡比真格文档 88` 使用的就是这套编号。若最后一页只剩一列条目，插件会自动并入前一页，避免额外发送一张近乎空白的图片。
